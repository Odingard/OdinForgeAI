...
export function buildEngineerReportPDF(report: EngineerReport): any {
  const logo = getLogoBase64();
  const now = new Date(report.generatedAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" });
  ...
  content.push({ canvas: [{ type: "line", x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 2, lineColor: COLORS.accent }] });
  content.push({ text: "ADVERSARIAL EXPOSURE VALIDATION", style: "reportType", margin: [0, 15, 0, 2] });
  content.push({ text: "Technical Assessment Report", style: "reportTitle", margin: [0, 0, 0, 5] });
  content.push({ text: `Engagement: ${report.engagementId} | Generated: ${now}`, style: "reportMeta" });
  content.push({ text: "", margin: [0, 0, 0, 15] });

  // ── Primary Attack Path ────────────────────────────────────────────
  if (report.primaryAttackPath) {
    const path = report.primaryAttackPath;
    content.push({ text: "PRIMARY ATTACK PATH", style: "sectionHeader" });
    content.push({ canvas: [{ type: "line", x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 1, lineColor: COLORS.border }] });

    content.push({
      columns: [
        { width: "*", text: [{ text: "Path: ", style: "label" }, { text: path.name || 'N/A', style: "bodyBold" }] },
        { width: 120, text: [{ text: "Confidence: ", style: "label" }, { text: (path.confidence || 'N/A').toUpperCase(), style: "bodyBold", color: COLORS.accent }] },
      ],
      margin: [0, 8, 0, 5],
    });

    if (path.narrative) {
      content.push({ text: path.narrative, style: "body", margin: [0, 0, 0, 10] });
    }

    if (path.steps?.length) {
      const stepRows = path.steps.map((s: any) => [
        { text: `${s.order}`, style: "tableCell", alignment: "center" },
        { text: (s.action || '').replace("[VALIDATED] ", ""), style: "tableCell" },
        { text: s.technique || '', style: "tableCell" },
        { text: s.mitreId || "", style: "tableCellMuted" },
      ]);
  ...
  // ── Chain Trace ────────────────────────────────────────────────────
  content.push({ text: "PHASE-BY-PHASE TRACE", style: "sectionHeader" });
  content.push({ canvas: [{ type: "line", x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 1, lineColor: COLORS.border }] });

  if (report.chainTrace.length > 0) {
    const traceRows = report.chainTrace.map(t => [
      { text: `${t.phaseIndex + 1}`, style: "tableCell", alignment: "center" },
      { text: t.displayName, style: "tableCell" },
      { text: t.status, style: "tableCell", color: t.status === 'completed' ? '#16a34a' : t.status === 'skipped' ? '#94a3b8' : COLORS.body },
      { text: `${t.findingCount}`, style: "tableCell", alignment: "center" },
      { text: t.durationMs > 0 ? `${Math.round(t.durationMs / 1000)}s` : '-', style: "tableCellMuted" },
    ]);
      ...
      table: {
        ...
        body: [
          [
            ...
            { text: "Findings", style: "tableHeader", alignment: "center" },
  ...
  // ── Findings Detail ────────────────────────────────────────────────
  content.push({ text: "VALIDATED FINDINGS", style: "sectionHeader" });
  content.push({ canvas: [{ type: "line", x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 1, lineColor: COLORS.border }] });
  content.push({ text: "", margin: [0, 0, 0, 5] });

  for (const finding of report.findingDetails) {
    const sevColor = finding.severity === 'critical' ? COLORS.gradeF : finding.severity === 'high' ? COLORS.gradeE : finding.severity === 'medium' ? COLORS.gradeD : COLORS.muted;

    content.push({
      columns: [
        { width: 60, text: finding.severity.toUpperCase(), style: "bodyBold", color: sevColor },
        { width: "*", text: finding.title.replace("[VALIDATED] ", ""), style: "bodyBold" },
        { width: 50, text: finding.mitreId || "", style: "tableCellMuted" },
      ],
      margin: [0, 5, 0, 2],
    });

    content.push({ text: finding.description, style: "body", margin: [60, 0, 0, 2] });

    if (finding.technique) {
      content.push({ text: `Technique: ${finding.technique}`, style: "tableCellMuted", margin: [60, 0, 0, 2] });
    }

    if (finding.httpEvidence) {
      content.push({
        text: `HTTP ${finding.httpEvidence.statusCode} — ${finding.httpEvidence.responseBodyPreview?.slice(0, 150) || 'response captured'}`,
  ...
  if (report.remediationPlan) {
    content.push({ text: "REMEDIATION PLAN", style: "sectionHeader", pageBreak: "before" });
    content.push({ canvas: [{ type: "line", x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 1, lineColor: COLORS.border }] });
    content.push({ text: "", margin: [0, 0, 0, 8] });

    const sections = [
      { title: "Immediate Actions", items: report.remediationPlan.immediate || [] },
      { title: "Pivot Disruption", items: report.remediationPlan.pivotDisruption || [] },
      { title: "Artifact Protection", items: report.remediationPlan.artifactProtection || [] },
      { title: "Privilege Boundary", items: report.remediationPlan.privilegeBoundary || [] },
      { title: "Monitoring & Detection", items: report.remediationPlan.monitoring || [] },
    ];
  ...
  // ── Remediation Diffs ──────────────────────────────────────────────
  if (report.remediationDiffs.length > 0) {
    content.push({ text: "REMEDIATION DIFFS", style: "sectionHeader" });
    content.push({ canvas: [{ type: "line", x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 1, lineColor: COLORS.border }] });

    for (const diff of report.remediationDiffs.slice(0, 10)) {
      content.push({
        columns: [
          { width: 60, text: diff.severity.toUpperCase(), style: "bodyBold", color: COLORS.gradeE },
          { width: "*", text: diff.findingTitle, style: "bodyBold" },
        ],
        margin: [0, 5, 0, 2],
      });
      content.push({ text: diff.recommendedState || 'See remediation plan above', style: "body", margin: [60, 0, 0, 8] });
    }
  }

  // ── Methodology ────────────────────────────────────────────────────
  content.push({ text: "METHODOLOGY", style: "sectionHeader" });
  content.push({ canvas: [{ type: "line", x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 1, lineColor: COLORS.border }] });
  content.push({
    columns: [
      { width: "*", text: [{ text: "Targets: ", style: "label" }, { text: report.methodologySummary.targetAssets.join(", "), style: "body" }] },
      { width: 120, text: [{ text: "Mode: ", style: "label" }, { text: report.methodologySummary.executionMode, style: "bodyBold" }] },
      { width: 80, text: [{ text: "Duration: ", style: "label" }, { text: `${Math.round(report.methodologySummary.totalDurationMs / 1000)}s`, style: "bodyBold" }] },
    ],
    margin: [0, 8, 0, 5],
  });
  content.push({ text: report.methodologySummary.evidenceStandard, style: "tableCellMuted", margin: [0, 0, 0, 15] });

  // ── Footer ─────────────────────────────────────────────────────────
  content.push({ canvas: [{ type: "line", x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 2, lineColor: COLORS.accent }] });
  content.push({
    text: "This report was generated by OdinForge AEV. All findings are backed by the OdinForge EvidenceContract — only PROVEN and CORROBORATED findings are included.",
    style: "footer",
    margin: [0, 10, 0, 5],
  });
  content.push({ text: "CONFIDENTIAL — Odingard Security | Six Sense Enterprise Services", style: "footer", alignment: "center" });

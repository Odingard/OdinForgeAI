...
func RunInstall(opts CLIOptions) error {
        ...
        if opts.RegistrationToken != "" {
                cfg.RegistrationToken = opts.RegistrationToken
        }
        if opts.TenantID != "" {
                cfg.TenantID = opts.TenantID
        }

        // Only prompt if no API key AND no registration token
        if opts.Interactive || cfg.ServerURL == "" || (cfg.APIKey == "" && cfg.RegistrationToken == "") {
        ...
        if cfg.APIKey == "" && cfg.RegistrationToken == "" {
                return fmt.Errorf("API key or registration token is required")
        }
...
func promptForConfig(cfg *InstallConfig) error {
        ...
        // Only prompt for API key or registration token if neither is provided
        if cfg.APIKey == "" && cfg.RegistrationToken == "" {
                fmt.Print("API Key (from Agents page, or leave empty for auto-registration): ")
                ...
                // If no API key, prompt for registration token
                if cfg.APIKey == "" {
                        fmt.Print("Registration Token (for auto-registration): ")
                        token, err := reader.ReadString('\n')
                        if err != nil {
                                return err
                        }
                        cfg.RegistrationToken = strings.TrimSpace(token)
                }
        }
